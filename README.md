...
# 回收用完的资源(operator 每 tick 的硬职责):分阶段、逐资源调用(见 operator-spec.md 分阶段 reap),带护栏。
bash scripts/reap.sh --session <tmux-session> --project <项目repo>          # verified 阶段:只 reap 会话,留 worktree+分支
bash scripts/reap.sh --worktree <path>        --project <项目repo>          # merged/rolled-back 阶段才 reap worktree
bash scripts/reap.sh --branch <name>          --project <项目repo>          # ……与分支(拒删 main/当前分支)
```
...
- `scripts/` — 英文代码,被 agent 调用:`discover-threads.py`(统一资源发现)、
  `board.py`(看板 jsonl 唯一写入者:upsert-thread / post-decision / resolve-decision / list-* /
