#!/usr/bin/env bash
#
# test-board-tui.sh -- offline tests for board.py `summary` and board-tui.py.
#
# The fixture board is populated ONLY through the board.py CLI (which doubles
# as a write-path test), then read back through the two terminal surfaces:
# the T0 one-line `summary` and the T1 headless `--render` frames. The final
# step deliberately corrupts threads.jsonl by hand to assert corruption is
# surfaced (stderr names file and line, exit 1), never hidden.
#
# The interactive curses layer needs a real TTY and is NOT covered here (see
# the NOTE at the end); only the headless surface and the exact subprocess
# the TUI shells for resolution are exercised.
#
...
# --- 8. resolve path (the exact subprocess the TUI shells) ---------------------
"$PY" "$BOARD_PY" resolve-decision --board "$BOARD" --id d-1 \
  --resolution "ship" >/dev/null || _die "resolve-decision d-1"
_ok "resolve-decision d-1 (ship)"
...
printf '\n%d checks passed\n' "$PASS"
printf 'NOTE: the interactive curses layer itself is NOT covered here (it needs a real TTY); only the headless --render surface, summary, and the board.py write path are verified.\n'
exit 0
