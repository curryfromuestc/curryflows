...
- **Bash 后台模式(`run_in_background`)起的进程(如 serve-board)挂进程、不挂对话**,长跑
...
- 看板服务:`curl --noproxy '*' -s -o /dev/null -w '%{http_code}' http://127.0.0.1:8787/`
  非 200 → Bash 后台模式重拉 `serve-board.py`(幂等:已在跑会 exit 0;绝不 `nohup`/`&`)。
...
- 落地人类已明确 resolve 的决策项:**若裁决实质修改了已封契约的约束(授权新面 / 放宽边界 /
  改 gate 条款),必须先重封契约——amend 条款 + `validate-contract` 过绿——再放行 worker**;
...
- **老化催办**:`list-decisions --open` 输出带 `age_hours`;超过 24h 的 open decision 必须在
  本 tick 摘要**置顶**重提(决策面是 pull 模型,人不看就永远不知道——催办是协调器的职责)。
...
- 向主 session 回**一条清晰、不糊弄的摘要**(schema 见 `board.md`:每线程状态/进展/预算、
  裁决含异议、未验证/风险、待决策项——`age_hours`>24 置顶催办、回收清单、backlog 水位;
  完整内容只给指针)。
...
0) 自检:若 {{PROJECT_DIR}}/.curryflows/pause 存在 → 只回复 "paused" 并结束。看板服务:
   curl --noproxy '*' -s -o /dev/null -w '%{http_code}' http://127.0.0.1:8787/ 非 200 → Bash
   后台模式重拉 python3 {{SKILL_DIR}}/scripts/serve-board.py --board $BOARD --port 8787
   (幂等,勿 nohup)。心跳 cron 消失/临近 7 天过期则重建。看板对比上一 tick 记录零变化、
...
3) 决策:pass/continue 自动处理;escalate → post-decision(同 id 重开用 --reopen)→
   ...
   interrupt-target.sh 软停(绝不 reap、绝不自裁 RULING)。落地人类已 resolve 项:裁决若实质
   ...
   入队,本 tick 不扩张。open decision 的 age_hours>24 → 摘要置顶催办。
...
6) 落盘+摘要:upsert-* 写回;非 no-op tick 把 {tick,summary,reviews,decisions_made,operator,
   ...
   的中文摘要(每线程状态/进展/预算、裁决含异议、未验证项、待决策项含超龄催办、回收清单、
   backlog 水位,完整内容只给指针)。本 tick 若有操作失误,修复落 durable 层(脚本/
   tick-prompt.md/契约),不能只写 tick 记录。然后结束本 turn。
```
