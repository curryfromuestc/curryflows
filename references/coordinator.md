...
`state`(线程当前状态)必须传给 review-panel:repro lens 据此判本次是进度审(L1/L2 可)还是
`committed→verified`(必须 L3,CANON [P])。`independence_tier` 回传本次复验实际达到的最高档——协调器
**只在它 = L3 时**才把 `committed` 线程置 `verified`(达不到 L3 一律不 verified)。
...
- **决定要不要起新 worker**:对 `state=ready` 的线程,定下分支 `curryflows/<thread-id>` + worktree +
  目标契约,交给 operator 起(尊重并发上限)。**seal-contract 前置(起 worker 前必过,两道门)**:已封契约
  落在 `<project>/.curryflows/contracts/<thread-id>.md`(`task-contracts/task.md` 填好的副本,
  `threads.jsonl` 的 `contract` 字段指向它),且必须**依次**通过:
  1. **文本门** `python3 <skillDir>/scripts/board.py validate-contract --file <contract>`(fail-closed:
     8 个必填字段齐且非空——outcome、verification、constraints、boundaries、iteration、budget、
     blocked_stop、**preconditions**);
  2. **执行门 environment-precondition dry-run(CANON [O])**:派一个 subagent 跑
     `bash <skillDir>/scripts/precondition-dryrun.sh --project . --base <base-ref> --contract <contract>`
     ——它在一个 base-ref 的 **throwaway worktree** 上真跑契约声称的每条 `preconditions` 检查
     (baseline 绿 / venv 可装 / 预期 drift),**任一 exit≠0 即不予封定**,契约打回 scoping 重修。
     协调器不亲自跑(CANON [J]),只据返回的 `{all_ok}` 决策。**这道门把"契约假设了没验证过的环境前提"挡在
     seal 前,而不是等 worker STEP-0 撞墙**(已观测:baseline 计数错、venv 无 in-venv pip、drift 门构造性
     不可满足——三个都会在此暴露、不污染 worker)。
  两门都过才 seal;校验不过不得起 worker。
...
1) 审核:调官方 Workflow 工具跑 review-panel.js——
   ...
   projectDir, threads:[{ thread_id, worktree, branch, codex_session, contract, worker_model, state }] } }),
   拿回 { reviews:[{ thread, branch, worktree, verdict(pass|continue|escalate), findings, dissent,
   independence_tier(L1|L2|L3|n/a), escalate[], resources[] }], escalations:[...] }。state 必传(repro lens
   据此判进度审 vs committed→verified);**只有 independence_tier=L3 才把 committed 线程置 verified**(CANON
   [P])。Workflow 内部 stage1 并发多 lens
...
2) 决策(你,薄):消费 Workflow 返回的收敛裁决(收敛已在 Workflow 内完成)——verdict=pass/continue
   ...
   .curryflows/contracts/<thread-id>.md 必过**两道 seal 门**:①board.py validate-contract(8 字段含
   preconditions);②派 subagent 跑 precondition-dryrun.sh --project . --base <base-ref> --contract <c>
   在 throwaway worktree 上真跑 preconditions 检查,all_ok 才封(CANON [O];任一 exit≠0 打回 scoping,
   不污染 worker)。所有看板写入一律走 scripts/board.py,**绝不手编 / 截断 / `>` jsonl**(破坏性操作前先读
   内容——已观测:未看就清空毁掉 durable 历史)。
