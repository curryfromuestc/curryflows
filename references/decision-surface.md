...
**协调器在 /loop 全程绝不调用 `AskUserQuestion` 或任何阻塞式提问来 gate 推进——整个过程不应出现一次 AskUserQuestion。** 阻塞弹窗违背"前进不等人";人类界面只有一个:durable 决策面(`decisions.jsonl`,人类经 board-tui 的 Decisions 视图或 `board.py` CLI 裁,CANON [R])+ 每-tick 摘要里的决策指针。
...
- **沉默 = 继续等,不是同意**:决策项在人类**明确** resolve 前一直 `open`,该线程一直停。**禁止的越权话术**(fail-closed,写进 resolution 即违规):① "人类知悉未异议 → 按默认推进"(沉默当同意);② "采纳推荐默认"而无人类明确批准;③ "异步 veto 窗口 / 先执行后否决"(把需人批的动作先做了再留否决窗口);④ 协调器对 `outward-irreversible` / `model-divergence` 标记项自行 "RULING / 裁定"。`resolve-decision` 的 `resolution` **必须能指向一次真实的人类动作**(board-tui 里的 resolve / CLI `resolve-decision` / 口头指示 / 消息里的明确批准),否则不得 resolve。
...
5. 人类只在这条队列上异步工作:在 board-tui 的 Decisions 视图逐条看 `summary` / `divergence` / `evidence`(`v` 键在内置 pager 里打开 artifact)/ `recommendation`,在 `options` 里选(输入编号即选中该项文本),`Enter` resolve / `x` reject;或直接用 CLI **`board.py resolve-decision --id <did> --resolution <text> [--status resolved|rejected]` 写回**——两条路殊途同归:`board.py` 是 `decisions.jsonl` 的唯一写入者(board-tui 的裁决也只是替你调它,CANON [R]),**绝不手编 `decisions.jsonl`**(手编易写坏行,而读回严格 fail-closed,一行坏整个决策面读回失败)——**前进不等人**。
6. 人类回复经 `board.py resolve-decision` 写回看板;下一个心跳 tick 读回 `list-decisions` 时落地对应 thread(落地两条路见第 5 节)。
