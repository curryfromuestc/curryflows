...
- **完成必须基于证据,不是模型自认为完成**:goal 不能因为模型觉得「大概做完了」就标记
  ...
  (见 goal-cookbook.md「completion must be evidence-based」)。VERIFICATION 的证据必须是 reviewer 能
  **L3 独立复跑**的(抹 venv + 删 `.so` + clean rebuild + 亲自跑,CANON [P]):worker 把可复跑的产物
  (log / reproducer / GOLD) checked-in,**但绝不能用自己、或自己 spawn 的 subagent(同谱系)跑的 replay
  冒充"独立复验"**——独立性来自 reviewer 另起的 L3,不是 worker 自证。

- **环境前提是 seal 前已 dry-run 过的(CANON [O])**:契约的 `preconditions`(baseline 绿 / venv 可装 /
  预期 drift)在 seal 前已由 seal-gate 在 fresh worktree 上真跑通过,故 STEP-0 可假定它们成立。**若 STEP-0
  发现某前提实际不成立**(seal-gate 漏网),那是环境缺陷,worker 按 BLOCKED_STOP 停下以 BLOCKER 上报,
  **绝不擅自 `git add -f` / 改 main / 静默绕过**去自救(已观测越权:draft agent 擅自 `git add -f` 解自己的
  blocker)。
