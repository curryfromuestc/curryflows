...
- **agentType**:`Explore`(只读 + Bash),由 `workflows/review-panel.js` stage1 拉起。reviewer
  **不改任何代码、不操作 tmux**(写动作全归协调器的操作步,规程见 `operator-spec.md`)。
...
reviewer 只**报告**这些;软停与回收由协调器在决策后内联执行(见 `operator-spec.md`)。
...
- **worker 自写/自改过的 gate 脚本,其输出一律不作为契约 gate 通过的证据**:验证器在被验证者的
  写权限内即自证(已发生:fp16 worker 自写 gateD_diff_audit.sh 把自己的越界改动加进白名单、删掉
  契约强制的审计 grep,exit 0 自报全绿)。reviewer 对照**契约文本里钉死的检查**亲自执行;发现
  gate 脚本与契约条款不一致,按"自证无效"记 blocker,不管其 exit code。
...
另有可从任意状态进入的 `blocked-human`;`session-reaped` 保留枚举、常规流不再经过):
...
- **`committed -> verified`(独立复验,**锁 L3**)**:worker 工作已 commit 到自己分支(`committed`,只保证
  ...
  复验,对应上面"3) 独立复验"。
  注:回收在终态一并执行(CANON [B] 修订)——`verified` 阶段**保留** codex 会话,合并冲突 / 回归
  才有活 worker 可驭回;`merged` / `rolled-back` 后才回收会话 + worktree + 分支。
