...
不了才升"自动消化。人类在终端表面(board-tui / `decisions.jsonl`,CANON [R])上异步处理,
**前进不等人**。
...
(`watch` / tmux status-right)、T1 `board-tui.py` 全屏 curses 看板按需开(zellij 浮动 pane /
tmux popup)。TUI 是 durable 看板的**只读渲染器 + 决策输入面**:写路径仅 ① `resolve-decision`
(经 `board.py` CLI)、② pause 文件开关;人类另保有 attach 到 worker pane 亲手 Esc 急停的既有
权利(不经 TUI 实现)。TUI **绝不执行生命周期操作**(起 / 驭 / commit / 合 main / 回收)——生命
周期写入者只有协调器一个;关闭 TUI 不影响推进。权威定义见 `board.md`「终端表面」。
...
- 人类异步视图 = 终端表面(`board.py summary` + `board-tui.py`,CANON [R],见 `board.md`
  「终端表面」),直接读上述 jsonl,不落额外渲染产物。
...
└──────────┬───────────┘  └──────────────────────┘
   ...
   协调器决策(③ 薄):一致+可判 → 自动;真分歧 → 升人类;落地已回复决策项
        ...
        ▼
   人类(异步看 board-tui / board.py summary,前进不等人)
```
