...
## 0. 上下文纪律:每 tick 全新上下文 + tick 内不灌大读(CANON [Q])

协调器上下文是**一次性资源**(CANON [Q],权威定义见 `coordinator.md`):session 级 cron 按节拍
注入 tick prompt,每 tick 从看板冷启动,tick 末 `arm-rebirth.sh` 安排延迟 `/clear` 整体丢弃——
跨 tick 状态只存在于 durable 看板(见 `board.md`)。tick 内的巨读仍必须隔离:巨型 transcript /
...
### 外层:协调器(cron 心跳 + 每 tick 全新上下文)= 调度

- tick **内部**是 agent 推理(审、裁、调度不可脚本化);tick 的**触发与重生**是确定性机械件:
  `/loop <间隔>` 的 session 级 cron 注入 tick prompt,尾步 `arm-rebirth.sh` 经 tmux server 延迟
  `/clear`(CANON [Q])。
- 每 tick 从看板读回全部状态(有界读),tick 末写回并整体丢弃上下文;无事可做即 no-op 收 tick。
...
- **机械操作 = 协调器内联执行**:detach 起新 /goal、`inject-steer.sh` 注入、`interrupt-target.sh`
  软停、commit、串行合 main、终态回收——输出都小(sha / exit code / session-id),fresh-per-tick +
...
的**触发与重生**(cron 注入、延迟 /clear)不能靠模型自觉——模型无法执行 `/clear`、长驻上下文必然
累积——所以交给确定性机械件(session cron + tmux server 一次性任务,CANON [Q])。
...
| 角色 | 能力 | 不能做 |
|---|---|---|
| 协调器(主 session,每 tick 全新上下文) | 推理、决策、调官方 Workflow 审核、**内联机械操作**(tmux 起/驭、commit、串行合 main、终态回收;驱动 codex 只经 inject-steer / interrupt-target)、写看板 jsonl(经 board.py)、**写文档类文件**(计划 / 契约 / 说明 / 覆盖矩阵 markdown) | 不读巨型 transcript/diff(隔离进 Workflow / subagent)、构建/测试输出不整读(重定向 + exit code)、**绝不自己写/改代码(源码 / 测试 / 脚本含 Workflow `.js`)并自己调试**(CANON [J]) |
...
任务契约(task-contracts/task.md)+ backlog.jsonl(每 tick 补货,CANON [M])
 │
 │  session cron(/loop <间隔>)按节拍注入 tick prompt —— 每 tick 全新上下文(CANON [Q])
 ▼
┌────────────────────────────────────────────────────────────────────┐
│ 协调器 tick:0 自检 → 1 读回 → 2 审核 → 3 决策 → 4 内联操作        │
│ → 5 补货 → 6 落盘 + 回摘要 + arm-rebirth                           │
│   (tmux server 延迟 /clear → 下一 tick 又从看板冷启动)            │
└───┬──────────────────────────┬──────────────────────────┬──────────┘
...
│ · 逐线程 pipeline    │  │ 强契约 budget+       │  │ + dashboard.html     │
│ · 多 lens+discover   │  │   blocked-stop       │  │ (durable 真相源)    │
...
每 tick 在全新上下文上先审核(官方 Workflow,巨型 transcript 隔离其内)→ 决策 → 内联操作 →
写 durable 看板 + 回摘要 + arm 重生;人类只在队尾异步看蒸馏后的决策面,决策不阻断推进。
