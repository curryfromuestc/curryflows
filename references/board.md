...
<project>/.curryflows/
  ...
  pause             # 存在即人类接管:tick 自停(人 touch 创建、rm 恢复;TUI 只显示不开关)
```
...
## 终端表面(CANON [R]):T0 常显摘要 + T1 只读看板 TUI

人类表面是终端原生的两层,jsonl 真相源不变:**T0** = `board.py summary` 一行摘要,用户自己
`watch` 在一个小 pane 里常显;**T1** = `board-tui.py` 全屏 curses 看板——**独立只读查看器**,
随时新开一个终端、一条命令即看,看完即关。
...
cfx-board                                        # 任意目录:cwd 上溯找板,找不到列注册表挑选
cfx-board --board <project>/.curryflows/board    # 显式指定
```

zellij 浮动 pane / tmux popup 保留为可选包装(T0 watch 行不变):

```bash
zellij run --floating --width 90% --height 85% -- cfx-board
tmux display-popup -w 90% -h 85% -E cfx-board
```

T0/T1 都由**用户**在自己的终端 / pane 里拉起——协调器 `start` 时只把这些命令打印给用户,不代拉。

### 无参发现:`--board` → cwd 上溯 → 注册表挑选

`--board` 是可选参数,解析顺序:① 显式 `--board`;② 从 cwd 逐级上溯找 `.curryflows/board`
目录;③ 读全局看板注册表(`~/.cache/curryflows/boards.jsonl`,环境变量 `CURRYFLOWS_REGISTRY`
可覆盖),只保留 board 目录仍存在的条目——恰好一个即直接打开,多个则进挑选视图(列项目名、
看板路径、`updated` 距今、与 `summary` 同字形的计数;`j`/`k` 移动、`Enter` 打开、`q` 退出);
注册表空 / 不存在 → stderr 提示(看板会在协调器写入时自动登记)+ exit 2。

注册表由 `board.py` 自动维护:每个**成功的**写子命令(upsert-thread / post-decision /
resolve-decision / record-tick / upsert-backlog)之后,把所涉看板登记进注册表(按看板绝对路径
去重,原子重写)。登记是 best-effort——注册表写失败绝不连累看板操作本身(stderr 一行警告,
退出码不变)。**注册表只是给独立查看器用的便利索引,不是真相**——看板文件仍是唯一真相源。
配套只读子命令 `board.py boards` 见下「board 写入」一节的 CLI 列表。

`board-tui.py` 另有 `--render threads|decisions|backlog|ticks|boards` 无头模式:不起 curses、向
stdout 打一帧该视图的纯文本(exit 0;jsonl 坏行 → stderr + exit 1;看板目录不存在按空看板渲染
表头),供无 TTY 的测试 / agent 读看板;不带 `--render` 且 stdout 非 TTY 时 fail-fast(exit 64,
提示改用 `--render`)。`--render boards` 即挑选表的无头 dump,无需 `--board`(注册表空 → exit 2
+ 同一 stderr 提示);其余 `--render` 视图省略 `--board` 时走同一解析顺序,但无头模式绝不画挑选
视图——无法自动定出唯一看板即 exit 2,提示显式传 `--board`。

### CANON [R]:board-tui 纯只读、零写路径(权威定义)

board-tui 是 durable 看板的**纯只读渲染器**:**零写路径**——不 resolve / reject 决策、不开关
pause、不执行任何生命周期操作(起 / 驭 / commit / 合 main / 回收)。生命周期与看板 jsonl 的
写入者只有协调器一个(经 `board.py`);关闭 TUI 对推进零影响。

**决策输入面 = 主 session 对话本身**:协调器每 tick 摘要把全部 open 决策项完整列出(见下
「每 tick 摘要 schema」),人类直接在主 session 回复,协调器执行 `board.py resolve-decision`
落盘——人不需要打开任何别的界面才能裁决,TUI 只是随手查看。pause 由人类直接
`touch` / `rm <project>/.curryflows/pause`(或让主 session 代执行),TUI 只在头部显示 PAUSED
状态。人类另保有既有的 Esc 急停权,但那是 attach 到 worker pane 亲手按的,不经 TUI 实现。

### board-tui 按键

全局:`1`/`2`/`3`/`4` 切视图(Threads/Decisions/Backlog/Ticks),`j`/`k` 与方向键移动选中,
`g`/`G` 跳首 / 末行,`r` 强制重读,`R` 跑资源发现(见下「刷新模型」),`b` 切换看板(重进注册表
挑选视图;注册表登记 ≥2 个看板才可用),`?` 帮助浮层(列全部按键),`q` 退出(`Q` 同义)。
所有视图的头部显示看板路径、与 `summary` 同字形的计数、PAUSED 指示;底部一行是动作结果 / 错误的
瞬态状态行。
...
**[2] Decisions**——只读;默认只显 open,`o` 切换显示全部;表列:id、barrier、thread、age(按
...
- `Enter` 在 pager 里看完整记录(JSON 缩进展开,同 Backlog)。

裁决不在 TUI 里做:人在主 session 对 tick 摘要直接回复,协调器执行
`board.py resolve-decision` 落盘(CANON [R])。
...
- `boards`——只读打印全局看板注册表(`~/.cache/curryflows/boards.jsonl`,env `CURRYFLOWS_REGISTRY`
  可覆盖)为 JSONL,每条补一个 `"exists": true|false` 字段(board 目录是否仍存在);注册表文件
  缺失 → 无输出、exit 0。不带 `--board`。注册表由每个**成功的**写子命令(upsert-thread /
  post-decision / resolve-decision / record-tick / upsert-backlog)best-effort 自动登记(登记失败
  仅 stderr 一行警告,不影响看板操作与退出码)——它只是给独立查看器用的索引,不是真相
  (见上「无参发现」)。
...
主 session 摘要的 durable 后备——摘要保持紧凑(决策项完整列出,其余只给指针),完整裁决在
这里,人类要深究时翻它:
...
| `decisions` | 每个 open 决策项**完整列出**:id、问题(summary/divergence)、options、recommendation、evidence 路径 | 主 session 就是决策面,人不该需要打开任何别的界面才能裁决;open 项每 tick 重列直到关闭(与 `age_hours` 催办配合,>24h 置顶);列的是决策项内容,**不贴 evidence 文件正文** |
