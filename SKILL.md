...
2. **防 runaway**:`discover-threads.py` 跨会话对账,杜绝"一个 codex /goal 跑 1.9 亿 token、3.7 天
   无人察觉"。
3. **durable 跨会话真相源 + 异步人类决策面**:`board.py` 看板 / `decisions.jsonl` 队列 / 终端看板
   (board-tui),人类跨天异步裁决、默认不阻断推进。
...
status-right)常显;**T1** = `scripts/board-tui.py` 全屏 curses 看板(zellij 浮动 pane / tmux
popup 随开随关),四视图(Threads/Decisions/Backlog/Ticks),在 Decisions 视图就地 resolve /
reject,`P` 开关 pause。TUI 写路径仅 resolve-decision(经 board.py)+ pause 开关,绝不执行生命
周期操作,关闭不影响推进。启动命令与按键表见 `references/board.md`「终端表面」。
...
board-tui 的 Decisions 视图 / `decisions.jsonl` 上异步处理。**"前进不等人"= 不弹窗打断你、其余无依赖线程不陪停;
...
- `start` — 在当前项目起协调器:宿主 tmux 会话 + 实例化 `./.curryflows/tick-prompt.md` + 挂
  ...
  T0/T1 两条看板启动命令(`watch … board.py summary` 常显行 + `board-tui.py` 的浮动 pane / popup
  行)**打印给用户**——它们是用户自己的 pane,协调器不代拉(CANON [R],命令原文见
...
- `board` — 看板 jsonl 的所有写入走 `scripts/board.py`(唯一写入者,原子 + fail-closed,见上);
  该动作 = 跑 `scripts/board.py summary --board ./.curryflows/board` 把当前一行状态回给用户,
  并把 T0(watch summary)/ T1(board-tui)两条启动命令打印给用户
  (见 `references/board.md`「终端表面」)。
