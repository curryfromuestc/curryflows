...
**curryflows 不重造循环引擎**:外层 `/loop` 定时心跳(session 级 cron,`/clear` 后仍按点触发、
跨天调度、非阻断推进)本身是 Claude Code 内置原语,任何会话都能直接用。curryflows 的**不可替代
...
## 上下文纪律:每 tick 全新上下文 + tick 内不灌大读(CANON [Q])

协调器上下文是**一次性资源**:session 级 cron 按节拍把 tick prompt 注入,每 tick 从看板冷启动,
tick 末用 `scripts/arm-rebirth.sh` 给自己安排延迟 `/clear` 整体丢弃——跨 tick 状态**只**存在于
durable 看板(CANON [Q],见 `references/coordinator.md`;看板见 `references/board.md`)。tick
内的巨读仍必须隔离:几百 MB 的 codex transcript / 大 diff 只进 review-panel Workflow / subagent
的上下文,随其消亡,协调器只收蒸馏裁决;协调器内联跑构建/测试一律输出重定向到文件、只读
exit code、失败才 tail。

## 三层控制流

1. **协调器(cron 心跳 + 每 tick 全新上下文)= 外层调度**:薄,做推理、决策、内联机械操作、写
   看板。`/loop <间隔>` 的 session 级 cron 把 tick prompt 按节拍注入,每 tick 从看板冷启动,tick
   末 `arm-rebirth.sh` 自我清空(CANON [Q])。协调器在 main 树上**只写文档**(计划 / 契约 / 说明 /
   覆盖矩阵),**绝不自己写 + 调代码**(源码 / 测试 / 脚本含 Workflow `.js`)——改码一律走 worker
   (worktree)/ fixer subagent / 动态 Workflow,小任务也照此(**CANON [J]**,见 `references/architecture.md`)。
...
职责分离:tick **内部**是开放式 agent 推理(审、裁、调度不可脚本化);tick 的**触发与重生**是
确定性机械件(session cron + tmux server 一次性任务),不靠模型自觉(CANON [Q])。

## tick:自检 → 读回 → 审核 → 决策 → 操作 → 补货 → 落盘+重生

每个 tick 在全新上下文上按此顺序执行(完整 runbook + tick prompt 模板见 `references/coordinator.md`):

0. **自检与短路**:pause 文件在 → 回 "paused" 即停;心跳/活性自检;看板零变化 → no-op 直达第 6 步。
1. **读回(全部有界)**:`list-threads --open` / `list-decisions --open` / `list-backlog` /
   `list-ticks --last K` + 北极星文档——上下文没有历史,读回就是全部记忆。
...
6. **落盘 + 回摘要 + arm 重生**:写回看板、record-tick、回一条清晰摘要(schema 见下),最后
   `arm-rebirth.sh` 安排自我 `/clear`,结束本 turn。
...
- `coordinator.md` — coordinator tick runbook(CANON [Q] fresh-per-tick)+ tick prompt 模板。
